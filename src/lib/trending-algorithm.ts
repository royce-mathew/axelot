...
export function calculateTrendingScore(doc: Document): number {
  const now = Date.now()
  const created =
    doc.created instanceof Timestamp ? doc.created.toMillis() : now
  const lastUpdated =
    doc.lastUpdated instanceof Timestamp ? doc.lastUpdated.toMillis() : now
...
export function shouldRecomputeTrendingScore(doc: Document): boolean {
  ...
  const now = Date.now()
  const lastComputed =
    doc.trendingLastComputed instanceof Timestamp
  ...
  const lastUpdated =
    doc.lastUpdated instanceof Timestamp ? doc.lastUpdated.toMillis() : 0
 ...
 */
export function batchComputeTrendingScores(
  docs: Document[]
): Map<string, number> {
