...
export async function signUpAction(formData: FormData) {
  try {
    ...
    // Validate with Zod
    const validatedData = await signUpSchema.parseAsync({ name, email, password });
    ...
    const credentialsRef = firebaseAdminFirestore.collection('credentials').doc(userRef.id);
